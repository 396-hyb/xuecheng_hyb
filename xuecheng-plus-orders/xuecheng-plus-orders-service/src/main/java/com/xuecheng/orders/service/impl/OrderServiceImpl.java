...
public class OrderServiceImpl implements OrderService {
    ...
    public void notifyPayResult(MqMessage message) {
        ...
        log.info("开始异步发送支付通知消息 - ID: {}, Content: {}", id, jsonString);

        try {
            // 构建消息
            Message<String> messageObj = MessageBuilder
                    .withPayload(jsonString)
                    .setHeader(RocketMQHeaders.KEYS, id.toString())
                    .setHeader(RocketMQHeaders.TAGS, PayNotifyConfig.PAYNOTIFY_TAG)
                    .build();

            // 异步发送消息，带回调处理
            rocketMQTemplate.asyncSend(
                    PayNotifyConfig.PAYNOTIFY_TOPIC + ":" + PayNotifyConfig.PAYNOTIFY_TAG,
                    messageObj,
                    new SendCallback() {
                        @Override
                        public void onSuccess(SendResult sendResult) {
                            log.info("异步发送消息成功 - ID: {}, MsgId: {}, QueueId: {}",
                                    id, sendResult.getMsgId(), sendResult.getMessageQueue().getQueueId());
                        ...
                        public void onException(Throwable e) {
                            log.error("异步发送消息失败 - ID: {}, Content: {}", id, jsonString, e);
                            // 可以选择重试或记录失败日志
                        }
                    }
            );
            ...
            log.error("发送消息异常 - ID: {}, Content: {}", id, jsonString, e);
        }
    }
